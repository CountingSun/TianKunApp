    ...
    [_netWorkEngine postWithDict:@{@"userId":[UserInfoEngine getUserInfo].userID} url:BaseUrl(@"find.resume.by.user_id") succed:^(id responseObject) {
        ...
        if (code == 1) {
            ...
            if ([objc isKindOfClass:[NSString class]] || objc == nil) {
                ...
                _isInfoFinish = [[[responseObject objectForKey:@"value"] objectForKey:@"resumeIsIntact"]boolValue];
                _isIntentionFinish = [[[responseObject objectForKey:@"value"] objectForKey:@"jobIntensionIsIntact"]boolValue];
...
- (void)previewButtobClick{
    ...
    if (!_isInfoFinish) {
        [self showErrorWithStatus:@"请先完善个人信息"];
        return;

    }
    if (!_isIntentionFinish) {
        [self showErrorWithStatus:@"请先完善求职意向"];
        return;


    }
    if (!_isIntroduceFinish) {
        [self showErrorWithStatus:@"请先完善个人简介"];
        return;
    ...
    [_netWorkEngine postWithDict:dict url:BaseUrl(@"update.resume.save") succed:^(id responseObject) {
        NSInteger code = [[responseObject objectForKey:@"code"] integerValue];
        self.view.userInteractionEnabled = YES;
        _previewButton.enabled = YES;
        ...
        if (code == 1) {
            ...
            [self showWithStatus:[responseObject objectForKey:@"msg"]];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    if (indexPath.row == 1) {
        ...
        vc.resumeID = _resumeInfo.job_intension_id;
        [self.navigationController pushViewController:vc animated:YES];
