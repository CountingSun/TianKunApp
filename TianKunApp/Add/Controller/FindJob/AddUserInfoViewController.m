...
- (void)editButtobClick:(UIButton *)button{
    ...
    if (!_resumeInfo.name.length) {
        [self showErrorWithStatus:@"请输入姓名"];
        return;
    }
    if (!_resumeInfo.birthday.length) {
        [self showErrorWithStatus:@"请选择生日"];

        return;
        
    }
    if (!_resumeInfo.degree.length) {
        [self showErrorWithStatus:@"请选择学历"];
        
        return;
        
    }
    if (!_resumeInfo.worked) {
        [self showErrorWithStatus:@"请输入工作经验"];
        return;
        
    }

    if (!_resumeInfo.phone.length) {
        [self showErrorWithStatus:@"请绑定电话号码"];
        return;
...
- (void)textFieldChange:(UITextField *)textField{
    if (textField.text.length>10) {
        [QMUITips showWithText:@"名字最长10个字"];
