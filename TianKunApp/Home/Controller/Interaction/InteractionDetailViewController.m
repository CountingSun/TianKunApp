...
- (void)getData{
    ...
    [self.netWorkEngine postWithDict:paraDict url:BaseUrl(@"Forums/selectForumbyid.action") succed:^(id responseObject) {
        ...
        if (code == 1) {
            ...
            [[[AddHistoryRecordNetwork alloc] init] addHistoryRecodeWithDataID:[_interactionID integerValue] dataType:7 dataTypeTwo:0 data_title:_interactionInfo.title data_sketch:_interactionInfo.content dataPictureUrl:@""];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    commentListViewController.block = ^{
...
- (IBAction)publicButtobClick:(id)sender {
    ...
    if ([UserInfoEngine isLogin]) {
        _publicButton.enabled = NO;
        
        _textView.text = [_textView.text stringTackOutBlank];

        
        if (!_textView.text.length) {
            [self showErrorWithStatus:@"请输入评论内容"];
            _publicButton.enabled = YES;
...
- (IBAction)shareButtonClick:(id)sender {
    [AppShared shareParamsByText:_interactionInfo.content images:@[[UIImage imageNamed:@"AppIcon"]] url:DEFAULT_SHARE_URL title:_interactionInfo.title];
}
