...
//获取评论数 回复数
- (void)getNumberWithInteractionInfo:(InteractionInfo *)interactionInfo indexPath:(NSIndexPath *)indexPath{
    [self.netWorkEngine postWithDict:@{@"id":interactionInfo.interactionID} url:BaseUrl(@"Forums/selectforumliuandping.action") succed:^(id responseObject) {
        NSInteger code = [[responseObject objectForKey:@"code"] integerValue];
        
        if (code == 1) {
            interactionInfo.hits_show = [[[responseObject objectForKey:@"value"] objectForKey:@"liu"] integerValue];
            interactionInfo.hfnum = [[[responseObject objectForKey:@"value"] objectForKey:@"ping"] integerValue];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    vc.reloadBlock = ^{
        ...
        [self getNumberWithInteractionInfo:info indexPath:indexPath];
        
//        info.hits_show = info.hits_show + 1;
//        [self.tableView reloadRowsAtIndexPaths:@[indexPath] withRowAnimation:UITableViewRowAnimationNone];
