...
//  ShopApp
//
//  Created by swqAdmin on 2017/6/23.
//  Copyright © 2017年 HMSoft. All rights reserved.
//
...
{
    if ([WQFile isFileExist:filePath]) {
        return YES;
    }
           ...
           isReplace:(BOOL)isReplace
{
    ...
    if ([WQFile isFileExist:toPath] && isReplace) {
        [WQFile deleteFile:toPath];
    }
                   ...
                   isReplace:(BOOL)isReplace
{
    ...
    for (NSString *path in contents) {
        ...
        if ([WQFile isFileExist:toFilePath] && isReplace) {
            [WQFile deleteFile:toFilePath];
        }
...
+ (double)calculteFileSzie:(NSString *)filePath
{
    ...
    if (dirContents == nil) {//文件夹为空时,dirContents不为nil但数目count为0,path为文件时,dirContents=nil
        ...
        for (NSString *dirName in dirContents) {
            fSize +=  [WQFile calculteFileSzie:[filePath stringByAppendingPathComponent:dirName]] ;
        }
    }
    ...
    if (dir != NULL) {
        ...
        while ((child = readdir(dir))!=NULL) {
            ...
            if (child->d_type == DT_DIR) {
                folderSize += [WQFile calculteFileSizeAtPath:childPath];
                // 把目录本身所占的空间也加上
...
+ (double)calculteFileSzieEx:(NSString *)filePath
{
    ...
    return [WQFile calculteFileSizeAtPath:[filePath cStringUsingEncoding:NSUTF8StringEncoding]];
}
...
+ (void)deleteFiles:(NSArray *)fileNames inPath:(NSString *)path
{
    ...
    if (dirContents == nil) {//文件夹为空时,dirContents不为nil但数目count为0,path为文件时,dirContents=nil
        if ([fileNames containsObject:[path lastPathComponent]]) {
            [WQFile deleteFile:path];
        }
    }
    else {
        for (NSString *dirName in dirContents) {
            if ([fileNames containsObject:dirName]) {
                [WQFile deleteFile:[path stringByAppendingPathComponent:dirName]];
            }
            else {
                [WQFile deleteFiles:fileNames inPath: [path stringByAppendingPathComponent:dirName]];
            }
        }
    }
}
